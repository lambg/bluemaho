...
class WorkerThread(Thread):
    ...
    def run(self):
        ...
        cmd_scan_out = subprocess.Popen(
            cmd_scan.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
        ...
        if len(cmd_scan_out[0]) > 16:
            ...
            for record in cmd_scan_out[0].split('\n\n')[1:-1]:
                ...
                for line in oui_database:
                    if oui_pat in line:
                        device[5] = line.split(
                            '\t')[2].strip()  # device_vendor
                oui_database.close()

                device[3] = record[1][13:].replace('[cached]', '')  # name
                device[4] = record[2][14:].replace(
                    "Device conforms to the ", "")  # class+type
                if len(record) > 3:
            ...
            if defconf.flag_do_sdp:  # get SDP info
                for device in devices:
                    sdp_services_list = []
                    cmd_sdp_browse = '%s %s' % (
                        defconf.cmd_sdp_browse.replace('@hci@', hci_dev), device[2])
                    cmd_sdp_browse_output = subprocess.Popen(cmd_sdp_browse.split(
                        ...
                        if len(cmd_sdp_browse_output[0]) == 31:
                            ...
                            try:
                                device[8] = cmd_sdp_browse_output[0].split('\n\n')[
                                    :-1]
                                # another way to remove first line?
                                device[8][0] = device[8][0][31:]
                            except:
                                device[8] = 'no services found.'
                    else:
...
class Frame_Main(wx.Frame):
    def __init__(self):
        wx.Frame.__init__(self, None, ID_MAINFRAME, "", (0, 0),
                          (404, 522), style=wx.CAPTION | wx.MINIMIZE_BOX)
  ...
  # WINDOW SETTINGS
        if verbose:
            print("[3] Drawing main window")
        self.SetBackgroundStyle(wx.BG_STYLE_CUSTOM)
        fontl = wx.Font(defconf.theme_fontsize, wx.TELETYPE,
                        wx.NORMAL, wx.NORMAL, False, u'Arial')
  ...
  # TOOLBAR
        ...
        self.toolbar.AddLabelTool(ID_MF_TOOLBAR_SCAN_START, "", wx.Bitmap(
            defconf.theme_ico_tb_run), shortHelp='start scan')
        self.Bind(wx.EVT_TOOL, self.StartInquiryScan,
                  id=ID_MF_TOOLBAR_SCAN_START)
        self.toolbar.AddLabelTool(ID_MF_TOOLBAR_SCAN_STOP, "", wx.Bitmap(
            defconf.theme_ico_tb_stop), shortHelp='stop scan')
        self.toolbar.EnableTool(ID_MF_TOOLBAR_SCAN_STOP, False)
        self.toolbar.AddSeparator()
        self.Bind(wx.EVT_TOOL, self.StopInquiryScan,
                  id=ID_MF_TOOLBAR_SCAN_STOP)
        self.toolbar.AddLabelTool(ID_MF_TOOLBAR_LOCATION, "", wx.Bitmap(
            defconf.theme_ico_tb_location), shortHelp='set your location')
        self.Bind(wx.EVT_TOOL, self.SetLocation, id=ID_MF_TOOLBAR_LOCATION)
        self.toolbar.AddLabelTool(ID_MF_TOOLBAR_LOCALDEV, "", wx.Bitmap(
            defconf.theme_ico_tb_dev), shortHelp='local HCI devices settings')
        self.Bind(wx.EVT_TOOL, self.DeviceSettings, id=ID_MF_TOOLBAR_LOCALDEV)
        self.toolbar.AddLabelTool(ID_MF_TOOLBAR_HANDBOOK, "", wx.Bitmap(
            defconf.theme_ico_tb_handbook), shortHelp='handbook')
        self.Bind(wx.EVT_TOOL, self.OnHandbook, id=ID_MF_TOOLBAR_HANDBOOK)

        self.toolbar.AddLabelTool(ID_MF_TOOLBAR_STAT, "", wx.Bitmap(
            defconf.theme_ico_tb_stat), shortHelp='statistics')
        ...
        self.toolbar.AddLabelTool(ID_MF_TOOLBAR_ABOUT, "", wx.Bitmap(
            defconf.theme_ico_tb_about), shortHelp='what\'s that? o_0')
        self.Bind(wx.EVT_TOOL, self.OnAbout, id=ID_MF_TOOLBAR_ABOUT)

        self.toolbar.AddLabelTool(ID_MF_TOOLBAR_EXIT, "", wx.Bitmap(
            defconf.theme_ico_tb_exit), shortHelp='exit')
  ...
  # CHECKBOXES
        # loop scan
        self.checkbox_loopscan = wx.CheckBox(
            panel, ID_MF_CHECKBOX_LOOP, "loop scan", (280, 5), (100, 17))
        self.checkbox_loopscan.SetValue(defconf.flag_loop_scan)
        self.checkbox_loopscan.Bind(
            wx.EVT_CHECKBOX, self.OnCheckLoop, id=ID_MF_CHECKBOX_LOOP)

        # sdp discovery
        self.checkbox_sdp = wx.CheckBox(
            panel, ID_MF_CHECKBOX_SDP, "get SDP info", (280, 22), (100, 17))
        self.checkbox_sdp.SetValue(defconf.flag_do_sdp)
        self.checkbox_sdp.Bind(
            wx.EVT_CHECKBOX, self.OnCheckSdp, id=ID_MF_CHECKBOX_SDP)

        # onnewdev - execute command if new device found
        self.checkbox_onnewdev = wx.CheckBox(
            panel, ID_MF_CHECKBOX_ONNEWDEV, "on new dev", (280, 39), (100, 17))
        self.checkbox_onnewdev.SetValue(defconf.flag_onnewdev)
        self.ReadCmdForOnNewDeviceFound(defconf.file_on_new_dev_found)
        self.checkbox_onnewdev.Bind(
            wx.EVT_CHECKBOX, self.OnCheckNewDev, id=ID_MF_CHECKBOX_ONNEWDEV)

        # log
        self.checkbox_log = wx.CheckBox(
            panel, ID_MF_CHECKBOX_LOG, "write to log", (280, 56), (100, 17))
        self.checkbox_log.SetValue(defconf.flag_log)
        self.checkbox_log.Bind(
            wx.EVT_CHECKBOX, self.OnCheckLog, id=ID_MF_CHECKBOX_LOG)

        # play sound if new device found
        self.checkbox_sound = wx.CheckBox(
            panel, ID_MF_CHECKBOX_SOUND, "play sound", (280, 73), (100, 17))
        self.sound = wx.Sound(defconf.theme_snd_onnewdev)
        self.checkbox_sound.SetValue(defconf.flag_sound)
        self.checkbox_sound.Bind(
            wx.EVT_CHECKBOX, self.OnCheckSound, id=ID_MF_CHECKBOX_SOUND)

        # show track info
        self.checkbox_trackinfo = wx.CheckBox(
            panel, ID_MF_CHECKBOX_TRACKINFO, "trackinfo", (280, 90), (100, 17))
        self.checkbox_trackinfo.SetValue(defconf.flag_trackinfo)
        self.checkbox_trackinfo.Bind(
            wx.EVT_CHECKBOX, self.OnCheckTrackinfo, id=ID_MF_CHECKBOX_TRACKINFO)

    # LOCAL HCI ADAPTERS CONTROLS
        self.hci_list = ['none']

        self.choicer_hci_scan = wx.Choice(panel, ID_MF_CHOICE_HCISCANDEV, wx.Point(
            285, 112), wx.Size(54, 20), choices=self.hci_list, style=wx.CB_DROPDOWN)
        self.choicer_hci_scan.Bind(
            wx.EVT_CHOICE, self.OnChoiseHciScanDevice, id=ID_MF_CHOICE_HCISCANDEV)
        self.choicer_hci_scan.SetSelection(0)
        wx.StaticText(panel, -1, "scan", wx.Point(342, 115), wx.Size(54, 20))

        self.choicer_hci_plug = wx.Choice(panel, ID_MF_CHOICE_HCITOOLDEV, wx.Point(
            285, 137), wx.Size(54, 20), choices=self.hci_list)
        self.choicer_hci_plug.Bind(
            wx.EVT_CHOICE, self.OnChoiseHciPlugDevice, id=ID_MF_CHOICE_HCITOOLDEV)
    ...
    # BOX FOR FOUND DEVICES
        self.monscreen = wx.grid.Grid(
            panel, -1, wx.Point(6, 5), size=(270, 152))
        self.monscreen.SetFont(fontl)
        self.monscreen.SetDefaultColSize(1, False)
        self.monscreen.SetDefaultCellFont(wx.Font(
            defconf.theme_fontsize_dev, wx.FONTFAMILY_SWISS, 90, wx.FONTWEIGHT_NORMAL, 0,  "Arial", wx.FONTENCODING_UTF8))
        ...
        self.monscreen.SetDefaultCellBackgroundColour(
            defconf.theme_bgcolor_displ)
        ...
        img_display = wx.StaticBitmap(
            panel, -1, wx.BitmapFromImage(img), pos=(5, 4))

        self.monscreen.Bind(
            wx.grid.EVT_GRID_CELL_LEFT_CLICK, self.OnSelectDevice)

    # BOX FOR SELECTED DEVICE INFO
        self.selected_dev_info = wx.StaticText(
            panel, -1, "", wx.Point(7, 162), wx.Size(376, 155))
    ...
    # TOOLS AND EXPLOITS CONTROLS
        ...
        for line in file:
            if line[0] != '#':
                self.expl[line.split('\t')[0]] = line.split('\t')[1].strip()
        file.close()
        self.expl_list = self.expl.keys()
        self.expl_list.sort()

        # create exploits choicer
        self.exploit_choiser = wx.Choice(panel, ID_MF_CHOICE_EXPLOIT, wx.Point(
            53, 344), wx.Size(190, 20), choices=self.expl_list, style=wx.CB_DROPDOWN)
        self.exploit_choiser.Bind(
            wx.EVT_CHOICE, self.OnExploitChose, id=ID_MF_CHOICE_EXPLOIT)
        ...
        for line in file:
            if line[0] != '#':
                self.tools[line.split('\t')[0]] = line.split('\t')[1].strip()
        file.close()
        self.tools_list = self.tools.keys()
        self.tools_list.sort()

        # create tools choicer
        self.tool_choiser = wx.Choice(panel, ID_MF_CHOICE_TOOL, wx.Point(
            246, 344), wx.Size(130, 20), choices=self.tools_list, style=wx.CB_DROPDOWN)
        self.tool_choiser.Bind(
            wx.EVT_CHOICE, self.OnToolChose, id=ID_MF_CHOICE_TOOL)

        # create run button
        self.run_button = wx.Button(
            panel, ID_MF_BUTTON_RUNTOOL, "run", wx.Point(340, 368), wx.Size(36, 20))
        self.run_button.Bind(wx.EVT_BUTTON, self.RunTool,
                             id=ID_MF_BUTTON_RUNTOOL)

        # create 'cmd-line'
        self.cmdline = wx.TextCtrl(
            panel, -1, "", wx.Point(2, 368), wx.Size(336, 20))

    # LOGGER
        WellcomeMsg = '> '+str(datetime.now())[:-7]+' ' + "bluemaho loaded.\n"
        self.logger = wx.TextCtrl(panel, -1, WellcomeMsg, wx.Point(2, 392),
                                  wx.Size(374, 80), wx.TE_MULTILINE | wx.TE_READONLY)
        self.logger.SetFont(wx.Font(defconf.theme_fontsize_log,
                            wx.TELETYPE, wx.NORMAL, wx.NORMAL, False, u'Arial'))
  ...
  ################
    ...
    def HciListRefresh(self):
        ...
        cmd_hciconfig_out = subprocess.Popen(
            defconf.cmd_hciconfig, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
    ...
    def SetLocation(self, event):
        dialog = wx.TextEntryDialog(
            self, "", "specify your location", defconf.user_location, style=wx.OK | wx.CANCEL)
    ...
    def OnNewDevFound(self, dev):
        ...
        cmd = cmd.replace(
            '@hci@', self.HCI_PLUG_DEVICE).replace('@hci_num@', self.HCI_PLUG_DEVICE[3:])
    ...
    def StartInquiryScan(self, event):
        if self.HCI_SCAN_DEVICE != 'none':
            ...
            self.PrintIntoLogger(
                'can\'t find usable adapter to perform scan!\n')
    ...
    def OnResult(self, event):
        if event.data is None:
            ...
            if event.data == 0:
                ...
                self.monscreen.DeleteRows(
                    0, self.monscreen.GetNumberRows(), True)
            else:
                devices = event.data
                ...
                for dev in devices:
                    ...
                    if dev[2] not in self.buffer_onnewdev:  # is it new device?
                        ...
                        label = "%s @%s found %s devices" % (
                            NAME, defconf.user_location, self.unique_devices_found)
                        self.SetLabel(label)
                    ...
                    if dev[8] != '':
                        for b in dev[8]:
                            ...
                            for c in b.split('\n'):
                                if 'Name:' in c:
                                    # garbage 'Bluetooth' before all profile names
                                    stri = c[14:].replace(
                                        'Bluetooth ', '').strip()
                                if 'Channel:' in c:
                                    if stri != '':
    ...
    def ShowInGrid(self, devices):
        ...
        for dev in devices:
            ...
            if defconf.flag_trackinfo:
                        ...
                        if hunt_name != 'n/a':
                            if hunt_name != dev[3]:
                                if hunt_name not in names_before:
                                    ...
                                    names_before_str = names_before_str + \
                                        string_splitted[4]+', '
                        # places
                        try:
                ...
                places_str = ' '
                l = []
                for k, m in wdict.items():
                    l.append((m, k))
                l.sort()
                l.reverse()
                for m, k in l:
                    places_str = places_str + k+'('+str(m)+')'+', '

                if names_before_str != '':
                    names_before_str = ' | names seen: '+names_before_str[:-2]
...
device vendor: %s\nchip: %s | lmp version: %s\nservices: %s" \
            ...
            # fxing somehow bug with invisibility of last column
            self.monscreen.SetCellTextColour(
                a, 3,  defconf.theme_bgcolor_displ)
    ...
    def OnSelectDevice(self, event):
        ...
        self.selected_dev_info.SetLabel(
            self.monscreen.GetCellValue(event.GetRow(), 3))
    ...
    def OnExploitChose(self, event):
        if self.exploit_choiser.GetCurrentSelection() == 0:
            ...
            if self.selected_device:
                cmdline = cmdline.replace('@bt_addr@', self.selected_device)
...
class Frame_LocalDev(wx.Frame):
    def __init__(self, parent, title):
        wx.Frame.__init__(
            self, parent, -1, "local HCI devices settings", pos=(60, 60), size=(310, 182))
        ...
        self.hci_choicer = wx.Choice(
            self.panel, ID_LD_CHOICE_HCI, (248, 6), (54, 20), choices=self.hci_list)
        self.hci_choicer.Bind(
            wx.EVT_CHOICE, self.OnSelectHci, id=ID_LD_CHOICE_HCI)

        self.Reinit()

        self.f_bdaddr = wx.StaticText(
            self.panel, -1, "Bd Address: none", (48, 30), (200, 10))
        self.button_bdaddr = wx.Button(
            self.panel, ID_LD_BUTTON_BDADDR, "set", (6, 25), (34, 20))
        self.button_bdaddr.Bind(
            wx.EVT_BUTTON, self.ChangeBdaddr, id=ID_LD_BUTTON_BDADDR)

        self.f_name = wx.StaticText(
            self.panel, -1, "Name: none", (48, 51), (200, 10))
        self.button_name = wx.Button(
            self.panel, ID_LD_BUTTON_NAME, "set", (6, 47), (34, 20))
        self.button_name.Bind(
            wx.EVT_BUTTON, self.ChangeName, id=ID_LD_BUTTON_NAME)

        self.f_class = wx.StaticText(
            self.panel, -1, "Class: none", (48, 72), (200, 10))
        self.button_class = wx.Button(
            self.panel, ID_LD_BUTTON_CLASS, "set", (6, 69), (34, 20))
        self.button_class.Bind(
            wx.EVT_BUTTON, self.ChangeClass, id=ID_LD_BUTTON_CLASS)

        self.f_manufa = wx.StaticText(
            self.panel, -1, "Manufacturer: none", (50, 92), (200, 10))

        # checkboxes
        self.checkbox_visib = wx.CheckBox(
            self.panel, ID_LD_CHECKBOX_VISIBLE, "visibility", (10, 109), (100, 17))
        self.checkbox_visib.Bind(
            wx.EVT_CHECKBOX, self.OnCheckVisib, id=ID_LD_CHECKBOX_VISIBLE)

        self.checkbox_auth = wx.CheckBox(
            self.panel, ID_LD_CHECKBOX_AUTH, "autentification", (10, 130), (100, 17))
        self.checkbox_auth.Bind(
            wx.EVT_CHECKBOX, self.OnCheckAuth, id=ID_LD_CHECKBOX_AUTH)
        self.checkbox_encrypt = wx.CheckBox(
            self.panel, ID_LD_CHECKBOX_ENCRYPT, "encryption", (110, 130), (100, 17))
        self.checkbox_encrypt.Bind(
            wx.EVT_CHECKBOX, self.OnCheckEncrypt, id=ID_LD_CHECKBOX_ENCRYPT)
        self.checkbox_secmgr = wx.CheckBox(
            self.panel, ID_LD_CHECKBOX_SECMGR, "secmanager", (190, 130), (100, 17))
        self.checkbox_secmgr.Bind(
            wx.EVT_CHECKBOX, self.OnCheckSecmgr, id=ID_LD_CHECKBOX_SECMGR)

        # buttons bottom
        self.showmaxinfo_button = wx.Button(
            self.panel, ID_LD_BUTTON_SHOWMAXINFO, "show max info", (90, 154), (90, 20))
        self.showmaxinfo_button.Bind(
            wx.EVT_BUTTON, self.OnShowMaxInfo, id=ID_LD_BUTTON_SHOWMAXINFO)
        self.refresh_button = wx.Button(
            self.panel, ID_LD_BUTTON_REFRESH, "refresh", (183, 154), (60, 20))
        self.refresh_button.Bind(
            wx.EVT_BUTTON, self.OnRefresh, id=ID_LD_BUTTON_REFRESH)
        self.close_button = wx.Button(
            self.panel, ID_LD_BUTTON_CLOSE, "close", (246, 154), (44, 20))
        self.close_button.Bind(
            wx.EVT_BUTTON, self.OnClose, id=ID_LD_BUTTON_CLOSE)
    ...
    def OnSelectHci(self, event):
        ...
        if self.current_hci != 'none':
            cmd_hciconfig_a = "%s -a %s" % (defconf.cmd_hciconfig,
                                            self.current_hci)
            cmd_hciconfig_out = subprocess.Popen(cmd_hciconfig_a.split(
            ), stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
            if cmd_hciconfig_out[0]:
                elements = cmd_hciconfig_out[0].split('\n\t')
                ...
                for element in elements:
                    if "BD Address" in element:
                        self.f_bdaddr.SetLabel(element[:29])
                    elif "Name" in element:
                        self.f_name.SetLabel(element)
                    elif "Class: 0" in element:
                        self.f_class.SetLabel(element)
                    elif "Manufacturer" in element:
                        self.f_manufa.SetLabel(element)
                    elif ("Device Class: " in element):
                        self.curr_cod_mj = element[14:]
                    elif ("Service Classes: " in element):
                        self.curr_cod_mi = element[17:]
                    if "AUTH" in element:
                        self.checkbox_auth.SetValue(True)
                    if "ENCRYPT" in element:
                        self.checkbox_encrypt.SetValue(True)
                    if "SECMGR" in element:
                        self.checkbox_secmgr.SetValue(True)
                    if "PSCAN" in element:
                        self.checkbox_visib.SetValue(True)
                    if "ISCAN" in element:
                        self.checkbox_visib.SetValue(True)
            else:
                self.hci_list = ['none']
    ...
    def ChangeBdaddr(self, event):
        ...
        dialog = wx.TextEntryDialog(self, "set bd_addr using 'bdaddr' tool",
                                    "change bdaddr", curr_bdaddr, style=wx.OK | wx.CANCEL)
        if dialog.ShowModal() == wx.ID_OK:
            cmd = "%s -i %s %s" % (defconf.cmd_bdaddr,
                                   self.current_hci, dialog.GetValue().strip())
            cmd_out = subprocess.Popen(
                cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
            if 'Address changed' in cmd_out[0]:
                if 'Warning!' in cmd_out[0]:
                    raiser = wx.MessageDialog(
                        None, "no verified support of bdaddr\nfor specified device!\nBD_ADDR may not be changed!", "bdaddr", wx.OK | wx.ICON_EXCLAMATION)
                    if raiser.ShowModal() == wx.ID_YES:
                        raiser.Destroy()
                cmd = "%s -a %s %s" % (defconf.cmd_hciconfig,
                                       self.current_hci, defconf.cmd_hciconfig_cmd_reset)
                cmd_out = subprocess.Popen(
                    cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
                if cmd_out != ('', ''):
                    raiser = wx.MessageDialog(
                        None, "can't reset device!\nReset it manually!", "bdaddr", wx.OK | wx.ICON_ERROR)
                    if raiser.ShowModal() == wx.ID_YES:
                        raiser.Destroy()
            self.OnSelectHci(None)
            if curr_bdaddr != self.f_bdaddr.GetLabel()[12:]:
                raiser = wx.MessageDialog(
                    None, "w00a! BD_ADDR of device changed!", "bdaddr", wx.OK | wx.ICON_INFORMATION)
            else:
                raiser = wx.MessageDialog(
                    None, "err.. bdaddr can't change\nBD_ADDR of specified device.\ntrying to do it with bccmd!", "bdaddr", wx.OK | wx.ICON_ERROR)
                if raiser.ShowModal() == wx.ID_YES:
                    raiser.Destroy()
                ...
                progress_win = wx.ProgressDialog(
                    "progress", "bccmd working...", progress_max, style=wx.PD_REMAINING_TIME | wx.PD_AUTO_HIDE)
                keepGoing = True
                count = 0
                ...
                if curr_bdaddr != self.f_bdaddr.GetLabel()[12:]:
                    raiser = wx.MessageDialog(
                        None, "w00a! BD_ADDR of device changed!", "bccmd", wx.OK | wx.ICON_INFORMATION)
                else:
                    raiser = wx.MessageDialog(
                        None, "err.. bccmd can't change\nBD_ADDR of specified device.", "bccmd", wx.OK | wx.ICON_ERROR)
            if raiser.ShowModal() == wx.ID_YES:
                raiser.Destroy()

    def ChangeName(self, event):
        dialog = wx.TextEntryDialog(self, "set new name for device", "change name", self.f_name.GetLabel()[
                                    7:-1], style=wx.OK | wx.CANCEL)
        if dialog.ShowModal() == wx.ID_OK:
            cmd = "%s -a %s %s %s" % (defconf.cmd_hciconfig, self.current_hci,
                                      defconf.cmd_hciconfig_cmd_name, dialog.GetValue())
            cmd_out = subprocess.Popen(
                cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
            if cmd_out != ('', ''):
                raiser = wx.MessageDialog(
                    None, "some error?", "change name of device", wx.OK | wx.ICON_ERROR)
                if raiser.ShowModal() == wx.ID_YES:
                    raiser.Destroy()
            self.OnSelectHci(None)
    ...
    def ChangeClass(self, event):
        ...
        dialog = wx.TextEntryDialog(self, dialog_message, "change CoD", self.f_class.GetLabel()[
                                    7:], style=wx.OK | wx.CANCEL)
        if dialog.ShowModal() == wx.ID_OK:
            cmd = "%s -a %s %s %s" % (defconf.cmd_hciconfig, self.current_hci,
                                      defconf.cmd_hciconfig_cmd_class, dialog.GetValue())
            cmd_out = subprocess.Popen(
                cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
            if cmd_out != ('', ''):
                raiser = wx.MessageDialog(
    ...
    def OnCheckAuth(self, event):
        if self.checkbox_auth.GetValue():
            cmd = "%s -a %s %s" % (defconf.cmd_hciconfig,
                                   self.current_hci, defconf.cmd_hciconfig_cmd_auth)
            subprocess.Popen(cmd.split(), stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE).communicate()
        else:
            cmd = "%s -a %s %s" % (defconf.cmd_hciconfig,
                                   self.current_hci, defconf.cmd_hciconfig_cmd_noauth)
            subprocess.Popen(cmd.split(), stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE).communicate()
    ...
    def OnCheckEncrypt(self, event):
        if self.checkbox_encrypt.GetValue():
            cmd = "%s -a %s %s" % (defconf.cmd_hciconfig,
                                   self.current_hci, defconf.cmd_hciconfig_cmd_encrypt)
            subprocess.Popen(cmd.split(), stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE).communicate()
            self.checkbox_auth.SetValue(True)
        else:
            cmd = "%s -a %s %s" % (defconf.cmd_hciconfig,
                                   self.current_hci, defconf.cmd_hciconfig_cmd_noencrypt)
            subprocess.Popen(cmd.split(), stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE).communicate()
    ...
    def OnCheckSecmgr(self, event):
        if self.checkbox_secmgr.GetValue():
            cmd = "%s -a %s %s" % (defconf.cmd_hciconfig,
                                   self.current_hci, defconf.cmd_hciconfig_cmd_secmgr)
            subprocess.Popen(cmd.split(), stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE).communicate()
        else:
            cmd = "%s -a %s %s" % (defconf.cmd_hciconfig,
                                   self.current_hci, defconf.cmd_hciconfig_cmd_nosecmgr)
            subprocess.Popen(cmd.split(), stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE).communicate()
    ...
    def OnCheckVisib(self, event):
        if self.checkbox_visib.GetValue():
            cmd = "%s -a %s %s" % (defconf.cmd_hciconfig,
                                   self.current_hci, defconf.cmd_hciconfig_cmd_piscan)
            subprocess.Popen(cmd.split(), stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE).communicate()
        else:
            cmd = "%s -a %s %s" % (defconf.cmd_hciconfig,
                                   self.current_hci, defconf.cmd_hciconfig_cmd_noscan)
            subprocess.Popen(cmd.split(), stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE).communicate()

    def OnShowMaxInfo(self, event):
        filename = '/dev/null'
        raiser = wx.MessageDialog(None, "save results to file?",
                                  "show max info about local HCI device", wx.YES_NO | wx.NO_DEFAULT | wx.ICON_QUESTION)
        if raiser.ShowModal() == wx.ID_YES:
            dialog = wx.FileDialog(
                self, "save as", os.getcwd(), style=wx.SAVE | wx.OVERWRITE_PROMPT)
            if dialog.ShowModal() == wx.ID_OK:
                filename = dialog.GetPath()
        raiser.Destroy()
        cmd = "%s %s %s %s" % (
            defconf.cmd_term, defconf.cmd_getmaxlocaldevinfo, self.current_hci, filename)
        subprocess.Popen(cmd.split(), stdout=subprocess.PIPE,
                         stderr=subprocess.PIPE).communicate()
...
class AboutBout(wx.Frame):
    def __init__(self, parent, title):
        wx.Frame.__init__(self, parent, -1, "about",
                          pos=(60, 60), size=(300, 390))
        ...
        wx.StaticText(self.panel, -1, message_ver,
                      (4, 320), (290, 180), wx.ALIGN_CENTER)
...
class Handbook(wx.Frame):
    def __init__(self, parent, title):
        wx.Frame.__init__(self, parent, -1, "handbook",
                          pos=(60, 60), size=(355, 400))
        ...
        self.hb_choiser = wx.Choice(self.panel, 2212, wx.Point(3, 6), wx.Size(
            348, 20), choices=self.hb_list, style=wx.CB_DROPDOWN)
        self.hb_choiser.Bind(wx.EVT_CHOICE, self.OnPageChose, id=2212)

        self.viewer = wx.TextCtrl(self.panel, -1, '', wx.Point(2, 30),
                                  wx.Size(349, 350), wx.TE_MULTILINE | wx.TE_READONLY)
        self.viewer.SetFont(wx.Font(defconf.theme_fontsize_hb,
                            wx.TELETYPE, wx.NORMAL, wx.NORMAL, False, u'Arial'))
...
class Stat(wx.Frame):
    def __init__(self, parent, title):
        wx.Frame.__init__(self, parent, -1, "statistics",
                          pos=(60, 60), size=(370, 130))
        ...
        self.fopenline = wx.TextCtrl(
            self.panel, -1, "", wx.Point(48, 4), wx.Size(260, 20))
        self.fopen_button = wx.Button(
            self.panel, ID_LD_BUTTON_CLOSE, "choose", (312, 4), (50, 20))
        self.fopen_button.Bind(
            wx.EVT_BUTTON, self.OnFopen, id=ID_LD_BUTTON_CLOSE)

        wx.StaticText(self.panel, -1, "output", (6, 36), (50, 12))
        self.fsaveline = wx.TextCtrl(
            self.panel, -1, "", wx.Point(48, 32), wx.Size(260, 20))
        self.fsave_button = wx.Button(
            self.panel, ID_LD_BUTTON_CLOSE, "choose", (312, 32), (50, 20))
        self.fsave_button.Bind(
            wx.EVT_BUTTON, self.OnFsave, id=ID_LD_BUTTON_CLOSE)

        wx.StaticText(self.panel, -1, "   from", (6, 64), (50, 12))
        self.fromdate = wx.TextCtrl(
            self.panel, -1, "", wx.Point(48, 60), wx.Size(76, 20))
        wx.StaticText(self.panel, -1, "to", (132, 64), (50, 12))
        self.todate = wx.TextCtrl(
            self.panel, -1, "", wx.Point(148, 60), wx.Size(76, 20))

        self.checkbox_hs = wx.CheckBox(
            self.panel, ID_ST_HOURS_STAT, "stat by hours", (240, 64), (100, 17))

        self.run_button = wx.Button(
            self.panel, ID_ST_BUTTON_RUN, "run", (240, 100), (44, 20))
        self.run_button.Bind(wx.EVT_BUTTON, self.OnRun, id=ID_ST_BUTTON_RUN)

        self.close_button = wx.Button(
            self.panel, ID_ST_BUTTON_CLOSE, "cancel", (300, 100), (44, 20))
        self.close_button.Bind(
            wx.EVT_BUTTON, self.OnClose, id=ID_ST_BUTTON_CLOSE)
    ...
    def OnRun(self, event):
        ...
        if self.fopenline.GetValue() == '' or self.fsaveline.GetValue() == '':
            raiser = wx.MessageDialog(
                None, "Not all required fields were filled?", "run", wx.OK | wx.ICON_ERROR)
            if raiser.ShowModal() == wx.ID_YES:
                raiser.Destroy()
        else:
            ...
            # uniq names
            file_output.write(
                "<table width=510><tr bgcolor=eeeeee><td><center><h3>unique names</h3></b></td></tr></table>\n")
            file_output.write(
            ...
            if hours_flag:
                file_output.write(
                    "<table width=510><tr bgcolor=eeeeee><td><center><h3>uniq devices per day</h3></b></td></tr></table>\n")
                file_output.write(
                    "<table width=500 cellpadding=0 cellspacing=0>\n")
                ...
                file_output.write('<b>'+old_date+' ' +
                                  day_of_week(old_date)+'</b><br>')
            ...
            for (a, b, c) in buf_time:
                try:
                    ...
                    buf[b] = 1
            file_output.write(
                "<table width=510><tr bgcolor=eeeeee><td><center><h3>uniq devices per hour</h3></b></td></tr></table>\n")
            file_output.write(
                "<table width=500 cellpadding=0 cellspacing=0>\n")
            l = []
            zoom = 1
            ...
            for (a, b, c) in buf_udevs:
                try:
                    ...
                    buf[c] = 1
            file_output.write(
                "<table width=510><tr bgcolor=eeeeee><td><center><h3>device vendors</h3></b></td></tr></table>\n")
            file_output.write(
                "<table width=500 cellpadding=0 cellspacing=0>\n")
            file_output.write(
            ...
            for (a, b, c) in buf_chiplmp:
                ...
                try:
                    ...
                    buf[b] = 1
            file_output.write(
                "<table width=510><tr bgcolor=eeeeee><td><center><h3>chip manufacturers</h3></b></td></tr></table>\n")
            file_output.write(
                "<table width=500 cellpadding=0 cellspacing=0>\n")
            file_output.write(
                "<tr><td></td><td width=50></td><td></td></tr>\n")
            ...
            for (a, b, c) in buf_udevs:
                try:
                    ...
                    buf[b] = 1
            file_output.write(
                "<table width=510><tr bgcolor=eeeeee><td><center><h3>class of devices</h3></b></td></tr></table>\n")
            file_output.write(
                "<table width=500 cellpadding=0 cellspacing=0>\n")
            file_output.write(
                "<tr><td></td><td width=50></td><td></td></tr>\n")
            ...
            for (a, b, c) in buf_chiplmp:
                try:
                    ...
                    buf[c] = 1
            file_output.write(
                "<table width=510><tr bgcolor=eeeeee><td><center><h3>LMP version</h3></b></td></tr></table>\n")
            file_output.write(
                "<table width=500 cellpadding=0 cellspacing=0>\n")
            file_output.write(
                "<tr><td></td><td width=50></td><td width=40></td></tr>\n")
            c = len(buf_chiplmp)
            l = []
            for k, m in buf.items():
                l.append((m, k))
            ...
            for a in buf_udevs_sdp:
                if a != '':
                    try:
                        ...
                        buf[a] = 1
            file_output.write(
                "<table width=510><tr bgcolor=eeeeee><td><center><h3>services</h3></b></td></tr></table>\n")
            file_output.write(
                "<table width=500 cellpadding=0 cellspacing=0>\n")
            file_output.write(
                "<tr><td></td><td width=50></td><td></td></tr>\n")
            ...
            raiser = wx.MessageDialog(
                None, "Statistics formed, sir!", "run", wx.OK | wx.ICON_INFORMATION)
            if raiser.ShowModal() == wx.ID_YES:
                raiser.Destroy()
                # ?self.Destroy()
    ...
    def OnFopen(self, event):
        ...
        if fopendialog.ShowModal() == wx.ID_OK:
            try:
                ...
                raiser = wx.MessageDialog(
                    None, "can't correctly parse specifyed input file! sorry, man..", "fopen", wx.OK | wx.ICON_ERROR)
                if raiser.ShowModal() == wx.ID_YES:
                    raiser.Destroy()
            ...
            if pair[0] == 'theme':
                ...
                for line_c in file_c:
                    if line_c[0] != '#':
                        pair_c = parse(line_c)
                        if pair_c[0] == 'bgcolor':
                            defconf.theme_bgcolor = pair_c[1]
                        elif pair_c[0] == 'fgcolor':
                            defconf.theme_fgcolor = pair_c[1]
                        elif pair_c[0] == 'bgcolor_displ':
                            defconf.theme_bgcolor_displ = pair_c[1]
                        elif pair_c[0] == 'fontsize':
                            defconf.theme_fontsize = int(pair_c[1])
                        elif pair_c[0] == 'fontsize_dev':
                            defconf.theme_fontsize_dev = int(pair_c[1])
                        elif pair_c[0] == 'fontsize_log':
                            defconf.theme_fontsize_log = int(pair_c[1])
                        elif pair_c[0] == 'fontsize_hb':
                            defconf.theme_fontsize_hb = int(pair_c[1])
                file_c.close()
            elif pair[0] == 'theme_ico_main':
                defconf.theme_ico_main = defconf.theme+pair[1]
            elif pair[0] == 'theme_ico_tb_exit':
                defconf.theme_ico_tb_exit = defconf.theme+pair[1]
            elif pair[0] == 'theme_ico_tb_about':
                defconf.theme_ico_tb_about = defconf.theme+pair[1]
            elif pair[0] == 'theme_ico_tb_dev':
                defconf.theme_ico_tb_dev = defconf.theme+pair[1]
            elif pair[0] == 'theme_ico_tb_run':
                defconf.theme_ico_tb_run = defconf.theme+pair[1]
            elif pair[0] == 'theme_ico_tb_stop':
                defconf.theme_ico_tb_stop = defconf.theme+pair[1]
            elif pair[0] == 'theme_ico_tb_location':
                defconf.theme_ico_tb_location = defconf.theme+pair[1]
            elif pair[0] == 'theme_ico_tb_handbook':
                defconf.theme_ico_tb_handbook = defconf.theme+pair[1]
            elif pair[0] == 'theme_ico_tb_stat':
                defconf.theme_ico_tb_stat = defconf.theme+pair[1]
            elif pair[0] == 'theme_img_dborder':
                defconf.theme_img_dborder = defconf.theme+pair[1]
            elif pair[0] == 'theme_snd_onnewdev':
                defconf.theme_snd_onnewdev = defconf.theme+pair[1]
            elif pair[0] == 'file_oui':
                defconf.file_oui = pair[1]
            elif pair[0] == 'file_tools_list':
                defconf.file_tools_list = pair[1]
            elif pair[0] == 'file_exploits_list':
                defconf.file_exploits_list = pair[1]
            elif pair[0] == 'file_logfile':
                defconf.file_logfile = pair[1]
            elif pair[0] == 'file_on_new_dev_found':
                defconf.file_on_new_dev_found = pair[1]
            elif pair[0] == 'file_handbook':
                defconf.file_handbook = pair[1]
            elif pair[0] == 'cmd_bccmd':
                defconf.cmd_bccmd = pair[1]
            elif pair[0] == 'cmd_bdaddr':
                defconf.cmd_bdaddr = pair[1]
            elif pair[0] == 'cmd_getmaxlocaldevinfo':
                defconf.cmd_getmaxlocaldevinfo = pair[1]
            elif pair[0] == 'cmd_hciconfig':
                defconf.cmd_hciconfig = pair[1]
            elif pair[0] == 'cmd_hcitool_scan':
                defconf.cmd_hcitool_scan = pair[1]
            elif pair[0] == 'cmd_hciconfig_cmd_auth':
                defconf.cmd_hciconfig_cmd_auth = pair[1]
            elif pair[0] == 'cmd_hciconfig_cmd_noauth':
                defconf.cmd_hciconfig_cmd_noauth = pair[1]
            elif pair[0] == 'cmd_hciconfig_cmd_encrypt':
                defconf.cmd_hciconfig_cmd_encrypt = pair[1]
            elif pair[0] == 'cmd_hciconfig_cmd_noencrypt':
                defconf.cmd_hciconfig_cmd_noencrypt = pair[1]
            elif pair[0] == 'cmd_hciconfig_cmd_secmgr':
                defconf.cmd_hciconfig_cmd_secmgr = pair[1]
            elif pair[0] == 'cmd_hciconfig_cmd_nosecmgr':
                defconf.cmd_hciconfig_cmd_nosecmgr = pair[1]
            elif pair[0] == 'cmd_hciconfig_cmd_piscan':
                defconf.cmd_hciconfig_cmd_piscan = pair[1]
            elif pair[0] == 'cmd_hciconfig_cmd_noscan':
                defconf.cmd_hciconfig_cmd_noscan = pair[1]
            elif pair[0] == 'cmd_hciconfig_cmd_name':
                defconf.cmd_hciconfig_cmd_name = pair[1]
            elif pair[0] == 'cmd_hciconfig_cmd_class':
                defconf.cmd_hciconfig_cmd_class = pair[1]
            elif pair[0] == 'cmd_hciconfig_cmd_reset':
                defconf.cmd_hciconfig_cmd_reset = pair[1]
            elif pair[0] == 'cmd_sdp_browse':
                defconf.cmd_sdp_browse = pair[1]
            elif pair[0] == 'cmd_term':
                defconf.cmd_term = pair[1]
            elif pair[0] == 'horizontal_pos':
                defconf.horizontal_pos = int(pair[1])
            elif pair[0] == 'vertical_pos':
                defconf.vertical_pos = int(pair[1])
            elif pair[0] == 'user_location':
                defconf.user_location = pair[1]
            elif pair[0] == 'flag_do_sdp':
                defconf.flag_do_sdp = bool(pair[1])
            elif pair[0] == 'flag_log':
                defconf.flag_log = bool(pair[1])
            elif pair[0] == 'flag_loop_scan':
                defconf.flag_loop_scan = bool(pair[1])
            elif pair[0] == 'flag_onnewdev':
                defconf.flag_onnewdev = bool(pair[1])
            elif pair[0] == 'flag_sound':
                defconf.flag_sound = bool(pair[1])
            elif pair[0] == 'flag_trackinfo':
                defconf.flag_trackinfo = bool(pair[1])
    file.close()
...
if verbose:
    print ("[0] Reading configuration from ") + CONFIG_FILENAME
ReadDefaultConfiguration(CONFIG_FILENAME)
if verbose:
    print ("[1] Using theme: ") + defconf.theme
